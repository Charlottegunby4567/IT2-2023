...
class Celle:
    def _init_(self, navn: str, radius: int, bildesti: str, x: int, y: int, farge: str):
        self.navn = navn
        self.radius = radius
    ...
    def spis(self, motspillere = None):
        print("Spis!")
        if motspiller:
...
motspillere: [
    Celle("SV", 25, "bilder/kristi.png", 500, 400, "purple"),
...
while True:
    ...
    for i in range(len(motspillere)-1, -1, -1):
        if spiller.ramme.collidedict(motspillere[i].ramme):
    ...
    for i in range(len(matbiter)-1, -1, -1):
        if spiller.ramme.collidedict(matbiter[i].ramme):
            spiller.spis()
            matbiter.pop(i)
